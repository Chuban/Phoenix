...
CNSFVHLLCViscousInternalSideFlux::CNSFVHLLCViscousInternalSideFlux(const InputParameters & parameters)
...
void
CNSFVHLLCViscousInternalSideFlux::calcFlux(unsigned int iside,
                                           unsigned int ielem,
                                           unsigned int ineig,
                                           ...
                                           std::vector<Real> & flux) const
{
  ...
  Real v2 = 1. / rho2;
  Real e2 = rhoe2 / rho2 - 0.5 * (uadv2 * uadv2 + vadv2 * vadv2 + wadv2 * wadv2);
  ...
  Real sm = (rho2 * vdon2 * dsv2 - rho1 * vdon1 * dsv1 + pres1 - pres2) /
            (rho2 * dsv2 - rho1 * dsv1);

  /// compute the Omega_l, Omega_r, and p^*

  Real omeg1 = 1.0 / (s1 - sm);
  Real omeg2 = 1.0 / (s2 - sm);
  ...
  RealVectorValue delta = mesh.elemPtr(ineig)->centroid() - mesh.elemPtr(ielem)->centroid();
  ...
  Real volvis = 0.;                                           // volume viscosity
  Real kinvis = 0.5 * _fp.mu(v1, e1) + 0.5 * _fp.mu(v2, e2);  // kinetic viscosity
...
void
CNSFVHLLCViscousInternalSideFlux::calcJacobian(unsigned int iside,
                                               unsigned int ielem,
                                               unsigned int ineig,
                                               const std::vector<Real> & uvec1,
                                               const std::vector<Real> & uvec2,
